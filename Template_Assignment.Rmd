...
moeite_per_jaar <- moeiteRondkomenAlleJaren %>%
  ...
  group_by(Jaar) %>%
  summarise(
    gem_moeite = mean(MoeiteMetRondkomen_32, na.rm = TRUE)
  )

ggplot(moeite_per_jaar, aes(x = Jaar, y = gem_moeite)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Gemiddelde moeite met rondkomen per jaar",
    x = "Jaar",
    y = "Moeite met rondkomen"
  ) +
  theme_minimal()
```
